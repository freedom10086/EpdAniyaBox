...
bmp_header_init_df(bmp_header *header,
                   const int width,
                   const int height) {
    header->bfSize = (sizeof(bmp_pixel_24) * width + BMP_GET_PADDING (width))
                     * abs(height);
...
void
bmp_pixel_init(bmp_pixel_24 *pxl,
...
void
bmp_img_init_df(bmp_img_24 *img,
                const int width,
                const int height) {
...
enum bmp_error
bmp_img_write(const bmp_img_24 *img,
              const char *filename) {
...
enum bmp_error
bmp_img_read(bmp_img_24 *img, const char *filename) {
