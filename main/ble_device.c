...
static const char remote_device_name[] = "24564-1"; // XOSS_VOR_S1091
...
static esp_bt_uuid_t remote_filter_service_uuid = {
        .len = ESP_UUID_LEN_16,
        .uuid = {.uuid16 = CYCLING_SPEED_AND_CADENCE_SERVICE_UUID,},
};
...
gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param) {
    ...
    switch (event) {
        ...
        case ESP_GATTC_SEARCH_RES_EVT: {
            ...
            ESP_LOGI(GATTC_TAG, "SEARCH RES: conn_id = %x is primary service %d uuid:%x", p_data->search_res.conn_id,
            ...
            if (p_data->search_res.srvc_id.uuid.len == ESP_UUID_LEN_16 &&
                p_data->search_res.srvc_id.uuid.uuid.uuid16 == CYCLING_SPEED_AND_CADENCE_SERVICE_UUID) {
                ESP_LOGI(GATTC_TAG, "service found UUID16: %x ", p_data->search_res.srvc_id.uuid.uuid.uuid16);
                get_server = true;
            ...
            switch (scan_result->scan_rst.search_evt) {
                case ESP_GAP_SEARCH_INQ_RES_EVT:  // Inquiry result for a peer device.
                    ...
                    if (adv_manufacturer_specific_type_len > 0) {
                        ESP_LOGI(GATTC_TAG, "searched MANUFACTURER SPECIFIC Len %d",
                                 adv_manufacturer_specific_type_len);
                                esp_log_buffer_hex(GATTC_TAG, adv_manufacturer_specific_type,
                                                   adv_manufacturer_specific_type_len);
